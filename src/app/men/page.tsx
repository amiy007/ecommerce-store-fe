    ...
    <main className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 py-8">
      <section className="max-w-5xl mx-auto px-4 mb-12">
        <div className="rounded-2xl bg-gradient-to-r from-blue-600 to-pink-500 p-8 text-white text-center shadow-lg mb-10">
          ...
          <p className="text-lg md:text-2xl mb-4">Discover the latest trends and bestsellers for men</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {menProducts.map(product => {
            const isWishlisted = wishlist.includes(product.id);
            return (
              <div key={product.id} className="bg-white rounded-xl shadow-lg p-6 flex flex-col items-center hover:scale-105 hover:shadow-2xl transition-all duration-300 border-2 border-transparent hover:border-gradient-to-r hover:from-blue-400 hover:to-pink-400 relative">
                ...
                <div className="flex gap-2 w-full mt-auto">
                  <button className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-700 transition" onClick={() => handleAddToCart(product.name)}>Add to Cart</button>
                  <button className="flex-1 bg-gray-200 text-blue-700 px-4 py-2 rounded-lg shadow hover:bg-blue-100 transition" onClick={() => setQuickViewProduct(product)}>Quick View</button>
                </div>
              </div>
            );
          })}
